import { C as he, j as Fe } from "chunk-5WINU2CB.mjs";
import { a as te } from "chunk-SPWSWTND.mjs";
import { a as ze } from "chunk-OAAFAMW7.mjs";
import { i as ce, m as le } from "chunk-E4V4QQI3.mjs";
import { Ch as De, U as Ee, eh as J, gi as ee, wh as qe } from "chunk-KIWT3OS5.mjs";
import { Nf as Oe, Oi as Le, b as de, mf as A } from "chunk-UVMW6WIE.mjs";
import {
  C as N,
  Ia as Ce,
  J as R,
...
import { o as ue } from "chunk-A4TNDN6S.mjs";
import { fa as ke, g as Re } from "chunk-HSCVLSVX.mjs";
import { fg as oe } from "chunk-IGDTGOWI.mjs";
import { b as xe, l as _, r as Te } from "chunk-DYNCKUFC.mjs";
import { ia as Q, s as G, t as Z, u as K, x as F } from "chunk-3GX5YKYT.mjs";
import { a as w } from "chunk-6PNPO5DW.mjs";
import { j as l, k as se, l as Ie } from "chunk-XELMBOBL.mjs";
    ...
    constructor(e) {
      ...
      l(this, "processRenderMainImage", () => {
        ...
        for (let t of this.scheduledRenderings) {
          ...
          if (n && (this.renderImage(n), n.show !== 0 && (e++, e > Ue))) break;
        }
...
var X,
  ne = class {
    ...
    hasTransforms(e) {
      return F() ? this.getCSSTransform(e) !== "none" : !this.getMatrix(e).isIdentity;
    }
    getMatrix(e) {
      ...
      return (
        [...o, e].forEach((c) => {
          let h = this.getSize(c),
            ...
            { x: Y, y: E, matrix: T } = ze(a, x, g, C),
            P = N($(Y + y), $(E + M)),
            U = N(S(P.x), S(P.y)),
            B = this.hasTransforms(c),
            H = A.hasNonTranslationTransform(f),
            b = st(c),
            { rect: z, paintDelta: L, translation: q } = it(i, P, h, f, B, b),
            W = Ze(B, H, b, P, U);
          ((i.x += W.x),
            (i.y += W.y),
            (i.paintX += L.x),
            ...
            (i.translationX += q.x),
            (i.translationY += q.y),
            (a = this.getCreates3dTransformContext(c) ? T : A.flatProjectionMatrix(T)),
    ...
    getTransformAncestors(e) {
      let t = [],
        n = e.parentElement;
      for (; !Te(n) && !Be(n) && n !== e.ownerDocument?.documentElement; )
...
function tt({ x: s, y: e, width: t, height: n }, r) {
...
function nt({ x: s, y: e, width: t, height: n }, r, o = []) {
...
function rt(s, e, { width: t, height: n }, r) {
  let { rotation: o, translation: i } = ye(r),
    ...
    m = A.convertPoint(h, N(u + t / 2, d + n / 2)),
    f = m.x - t / 2,
    y = m.y - n / 2;
...
function it(s, e, t, n, r, o) {
  ...
  if (K()) {
    let y = re(d),
      M = R.offset(y, { x: u.x + a.x, y: u.y + a.y });
    if (r) {
      let { paintedRect: I, snappedLayoutRect: p, translation: x } = tt(d, n),
        C = R.offset(I, { x: u.x + a.x, y: u.y + a.y }),
        g = R.offset(p, x);
      return ((i = { x: g.x - I.x, y: g.y - I.y }), { rect: C, paintDelta: i, translation: x });
    }
    return { rect: M, paintDelta: i, translation: { x: 0, y: 0 } };
  }
  if (Z() || G()) {
    let y = re(d),
      M = R.offset(y, { x: u.x + a.x, y: u.y + a.y });
    if (r) {
      let { paintedRect: I, snappedLayoutRect: p, translation: x } = nt(d, n, o),
        C = R.offset(I, { x: u.x + a.x, y: u.y + a.y }),
        g = R.offset(p, x);
      return ((i = { x: g.x - I.x, y: g.y - I.y }), { rect: C, paintDelta: i, translation: x });
    }
  ...
  if (r) {
    let y = R.offset({ x: s.x + S(e.x), y: s.y + S(e.y), width: m.width, height: m.height }, a),
      M = R.offset(rt(s, e, t, n), a);
    return (
      (i = { x: y.x - M.x, y: y.y - M.y }),
      { rect: M, paintDelta: i, translation: { x: 0, y: 0 } }
    );
  }
...
function ot() {
  return F() ? 60 : G() ? 100 : 64;
}
function at(s) {
  let [e, t] = s.split(" ").map((n) => parseFloat(n));
  if (!(!oe(e) || !oe(t))) return { x: e, y: t };
}
var v = Q("LayoutMeasureQueue"),
  dt = new DOMMatrixReadOnly(),
  We = F() ? 1 : 0,
  Ge = class {
    constructor({
    ...
    remeasureChildrenWithDOMRects(e) {
      if ((v.trace("remeasureChildrenWithLayout:", e.id), !!e.children && !ae(e)))
        for (let t of e.children)
          (Pe(t) && !t.usesDOMRectCached()) ||
            this.remeasureCachedElements(t, (n, r) => {
              v.trace("remeasureChildrenWithLayout, child:", n, r);
            });
    }
    remeasureLayoutChildren(e) {
      if ((v.trace("remeasureLayoutChildren:", e.id), !!ae(e)))
        for (let t of e.children)
          this.remeasureCachedElements(t, (n, r) => {
    ...
    process({ scopeId: e, zoom: t, offsetX: n, offsetY: r, tree: o, groundNodesInViewport: i }) {
      ...
      if (
        ...
        return [];
      let h = dt.scale(t).translateSelf(-n, -r),
        m = [],
        f = new Map(),
        y = new ne();
      ...
      let M = ut(this.measureRequests.values(), o, h),
        I = new Set();
      for (let [p, x] of this.measureRequests) {
        ...
        if (!T) continue;
        if (!p.isConnected) {
          v.trace("measureRequest: not connected", g);
          continue;
        }
        E
          ? (w(ce(g), "Layout templates should only have a primary rendering"),
            (T.cache.elementByLayoutTemplateSelector ??= new Map()),
            T.cache.elementByLayoutTemplateSelector.set(E, p))
          : ((T.cache.elementByRenderId ??= new Map()), T.cache.elementByRenderId.set(g, p));
        let P = M.get(C),
          U = P.matrix,
          B = P.rect;
        if (!i.has(P.id)) {
          (this.addRefinementRequest(g, { element: p, groundNodeId: P.id, measureRequest: x }),
            v.trace("measureRequest: refine", g));
          continue;
        }
        let H;
        if (Y) {
        ...
        let b = f.get(p);
        (b
          ? v.trace("measureRequest: cached", g, b)
          : ((b = y.getParentRelativeRect(p, U, B, !E)),
            f.set(p, b),
            v.trace("measureRequest: getParentRelativeRect", g, b)),
          t <= We &&
            this.addRefinementRequest(g, { element: p, groundNodeId: P.id, measureRequest: x }));
        let z = { ...b };
        (E ? Object.assign(z, y.getCachedAttributes(p)) : (z.childrenRects = H),
          T &&
            (be(T) ? this.richTextRenderIds.add(T.id) : we(T) && this.resizeObserver?.observe(p),
            !E &&
              !R.equals(T.cache.getRawDOMRect(g) ?? null, b) &&
              (I.add(g), T.cache.setRawDOMRect(g, b))),
...
function dn(s) {
  return s.resultKeyPath.length > 0 && ve(s);
}
function Xe(s, e) {
  let t = e.value;
  if (
    (Se(t) &&
      (w(!Me(t), "Computed values should not be possible in fetch fallback"),
...
export { Ae as a, It as b, Ve as c, dn as d, Xe as e, un as f, Ge as g };
//# sourceMappingURL=chunk-ONXZEOD4.mjs.map
