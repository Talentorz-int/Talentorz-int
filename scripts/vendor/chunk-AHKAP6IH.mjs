import { a as Y } from "chunk-CQACNNW4.mjs";
import { j as V, k as _ } from "chunk-A3QS6XPJ.mjs";
import { a as H } from "chunk-HRM2PG23.mjs";
import { La as j } from "chunk-424ANBSC.mjs";
import { th as q, uh as B, vh as L } from "chunk-CLNV2WTB.mjs";
import { I as F } from "chunk-Q4DBFMRT.mjs";
import { c as G, d as J } from "chunk-QGHEOP6N.mjs";
import { d as K } from "chunk-JG5QFAU7.mjs";
import { b as k } from "chunk-SJT3LDOF.mjs";
import { b as o } from "chunk-6FJY55U7.mjs";
import { g as D } from "chunk-IAA62EI4.mjs";
import { a as R } from "chunk-RNHTTH2C.mjs";
import { a as W } from "chunk-LEYNIZPU.mjs";
import { o as M } from "chunk-RJAQ7ZXH.mjs";
import { l as z } from "chunk-DYNCKUFC.mjs";
import { a as x } from "chunk-SJONTHBI.mjs";
import { k as O } from "chunk-WJBO7C5H.mjs";
import { ia as N } from "chunk-3GX5YKYT.mjs";
import { a as $, b as T } from "chunk-6PNPO5DW.mjs";
import { a as A } from "chunk-HMF7T2NG.mjs";
import { e as w, j as I } from "chunk-XELMBOBL.mjs";
var l = w(x(), 1);
var E = w(W(), 1),
  g = class n {
    constructor(e) {
    ...
    static canceled() {
      return new n(L("cancelled"));
    }
    static empty() {
      return new n(L("empty"));
    }
    static files(e) {
      return new n(B(e));
    }
...
function te(
  ...
  (0, l.useImperativeHandle)(i, () => ({
    show(p, P) {
      return new Promise((v, b) => {
        if (u.current) {
          let [, m] = u.current;
          m(Error("Multiple calls to show()"));
        }
        if (
          ((u.current = [v, b]),
          ...
          ((u.current = null), v(g.canceled()));
          return;
        }
        (p ? (c.current.accept = p.join(",")) : c.current.removeAttribute("accept"),
          P ? (c.current.multiple = !0) : c.current.removeAttribute("multiple"),
          c.current.click());
      });
    },
  }));
  let S = (0, l.useCallback)(
      (p) => {
          ...
          s === void 0 || se(m.size, s, m.type)
            ? (m.size / 1e6 > a &&
                ...
                k({
                  ...
                  action: {
                    title: "OK",
                    onClick: () => k({ type: "remove", key: "file-picker-large-file-detected" }),
                  },
                }),
              b.push(m))
            : r
        ...
        (b.length > 0 ? P(g.files(b)) : P(g.empty()), d.current?.reset());
      },
      [s, r, t]
    ),
...
var ne = (0, l.forwardRef)(te);
var Q = class {
  constructor(e, s) {
    this.api = e;
    this.socket = s;
    I(this, "emitter", new O());
    s.onMessage(async (t) => {
  ...
  async enqueueKitScreenshots(e) {
    return this.api.enqueueKitScreenshots(e);
  }
};
var h = w(x(), 1);
...
var y = N("useAPI"),
  U = class {
    constructor(e, s, t = R) {
      ...
      this.projectId = s;
      this.dispatch = t;
      I(this, "apiBaseURL", A().api);
      I(this, "packagesPerPage", 36);
    }
    wait(e) {
      o.wait(e);
    }
    normalizeRole(e) {
      return e === "contentCollaborator" && !M.isOn("contentEditor") ? "collaborator" : e;
    }
    getACL() {
      o.get(`/web/projects/${this.projectId}/acl/`, {
        contentCollaboratorEnabled: M.isOn("contentEditor"),
      })
        ...
        .catch((e) => y.error("Failed to get ACL:", e));
    }
    setInitialProject(e) {
      e.then((s) => {
        this.dispatch({ type: "setProject", project: s });
      }).catch((s) => y.error("Failed to set initial project:", s));
    }
    getProject() {
      o.get(`/web/projects/${this.projectId}`, {
        includeUsageDataV2: "true",
        includeAiCreditLimit: M.isOn("agent") ? "true" : void 0,
      })
        .then((e) => {
          this.dispatch({ type: "setProject", project: e });
        })
        .catch((e) => y.error("Failed to get project:", e));
    }
    async pollProject({ intervalMillis: e, attempts: s, stopCondition: t }) {
      let r = await H(this.projectId, { intervalMillis: e, attempts: s, stopCondition: t });
      return (
        r.status === 0 &&
    ...
    async updateUserPermissions(e) {
      let s = await _(this.projectId, e);
    ...
    async updateProject(e, s = !0) {
      ...
      try {
        (await K(this.projectId, e), this.getProject(), this.notifyProjectChange("metadata"));
      } catch (t) {
        throw (y.error("Failed to update project:", t), t);
      }
    }
    ...
    async duplicateAssets(e, s) {
      if (this.projectId === s)
        return (y.warn("Attempted to duplicate assets for current project"), []);
    ...
    async listKits() {
      return await o.get("/modules/v1/kits/", { projectId: this.projectId });
    }
    async listNamespaces() {
    ...
    async lookUpModules(e) {
      let s = q() ? "?respectIncludeStatus=true" : "";
      return o.post(`/modules/v1/modules/batch/lookup/${s}`, e);
    }
    ...
    async saveModules({ batch: e, copyOnWrite: s }) {
      let t = new FormData();
      return (
        await Promise.all(e.map((a) => this.addModuleRequestToForm(a, t))),
        await o
          .postRaw(`/modules/v1/modules/batch/saves/${this.modulesCopyOnWriteParam(s)}`, t)
          .then((a) => a.json())
      );
    }
    async addModuleRequestToForm(e, s) {
      ...
      (j && window.CompressionStream && (r.transferEncoding = "gzip"),
        s.append(
          "metadata",
          JSON.stringify({
            ...r,
        ...
        await Promise.all(
          t.map(async (i) => {
            ...
            $(!z(d), "File needs content or bytes");
            let c = new Blob([d]);
            if (j && window.CompressionStream) {
              let u = new window.CompressionStream("gzip"),
                S = c.stream();
              c = await new Response(S.pipeThrough(u)).blob();
              let f = d.length - c.size,
                C = (f / d.length) * 100;
              y.debug("Saved", f, "bytes", `(${C.toFixed(1)}%)`, "compressing", i.name);
            }
            s.append(`files[${a}]`, new File([c], i.name));
          })
        ));
    }
    modulesCopyOnWriteParam(e = !1) {
      return e || q() ? `?copyOnWrite=${this.projectId}` : "";
    }
    async enqueueKitScreenshots(e) {
      let s = new FormData(),
        t;
      if (j && window.CompressionStream) {
        let r = new window.CompressionStream("gzip"),
          a = new Response(e.kitPageContent).body;
        $(a, "Section page content body is not readable");
        let i = await new Response(a.pipeThrough(r)).blob();
        t = new File([i], "kitPage.html.gz", { type: "application/gzip" });
      } else t = new File([e.kitPageContent], "kitPage.html", { type: "text/html; charset=utf-8" });
      (s.append("sectionPage", t),
        s.append("sectionIds", JSON.stringify(e.ids)),
        await o.postRaw(
          `/modules/v1/modules/${e.moduleId}/saves/${e.saveId ?? "draft"}/kit-screenshots/`,
          s
        ));
    }
...
function Oe(n, e, s) {
  let t = (0, h.useMemo)(() => new U(n, e.projectId, s), [n, e.projectId, s]);
  (0, h.useEffect)(() => {
    t.getACL();
  }, [t]);
  let r = (0, h.useRef)(e);
...
export { ne as a, Q as b, U as c, Oe as d, Be as e, He as f };
//# sourceMappingURL=chunk-AHKAP6IH.mjs.map
