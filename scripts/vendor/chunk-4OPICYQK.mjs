import { So as F, Uo as T, ap as W } from "chunk-C45BQYIA.mjs";
import { a as k } from "chunk-OETKI2VS.mjs";
import { b as L } from "chunk-NYGWFQ3Z.mjs";
import { a as v } from "chunk-LEYNIZPU.mjs";
import { a as w } from "chunk-SJONTHBI.mjs";
import { Ba as l, ia as C, sa as m, ta as u } from "chunk-3GX5YKYT.mjs";
import { a as I } from "chunk-6PNPO5DW.mjs";
import { e as E, j as R } from "chunk-XELMBOBL.mjs";
var d = E(w(), 1);
var j = E(v(), 1),
  V = C("useModuleState");
function Z(e, t) {
  let [n, r] = (0, d.useState)(() => A(t, e)),
    o = (0, d.useCallback)(
      ({ children: a }) =>
        (0, j.jsx)(b, { moduleIdentifier: t, modulesRuntime: e, setInternalState: r, children: a }),
      [t]
    );
  return (
    d.default.useEffect(() => {
      if (!t) return;
      if ((n.moduleIdentifier && t !== n.moduleIdentifier && r(A(t, e)), u(t)))
        return e.subscribeToLocalModuleExports(t, (i, f, D, s) => {
          r(
            f === "fast-refresh"
              ? (c) => {
                  let S = M(i, t, e.componentLoader, f);
                  return c.status === "success" &&
                    s &&
                    s.error === void 0 &&
                    c.moduleIdentifier === t &&
                    U(c.definition?.annotations, S.definition?.annotations) &&
                    $(c.definition?.properties, S.definition?.properties)
                    ? c
                    : S;
                }
              : M(i, t, e.componentLoader, f)
          );
        });
      m(t) &&
        r((i) =>
          i.status === "loading" || i.moduleIdentifier !== t ? i : p(i.definition, t, "server")
        );
      let a = !0;
      return (
        P(t, e)
...
function A(e, t) {
  ...
  if (u(e)) {
    ...
    return M(n, e, t.componentLoader, r);
  } else if (m(e)) {
    let n = t.componentLoader.componentForIdentifier(e) ?? t.componentLoader.errorForIdentifier(e);
    return n ? p(n, e, "server") : { status: "loading", moduleIdentifier: e };
  }
  L(e);
}
function U(e, t) {
...
async function P(e, t) {
  ...
  return (I(r, "expected either component or error definition to be present for", e), r);
}
function M(e, t, n, r) {
  if (!e) return { status: "loading", moduleIdentifier: t, kind: void 0 };
  let o = n.renderableComponentForIdentifier(t) ?? n.errorForIdentifier(t);
  return p(o, t, r);
}
function p(e, t, n) {
  if (!e) {
    e = W(t, "Unknown");
    let { title: r, message: o } = x(e, void 0);
    return {
      status: "error",
      moduleIdentifier: t,
      title: r,
...
function K(e, t, n) {
  return u(e) ? (e.type === "codeFile" ? `Error in ${q(t)}` : n ? `Error in ${n}` : h) : h;
}
function q(e) {
...
function re(e, t) {
  let n = "Unknown error";
  (t !== void 0 && (n = t instanceof Error ? t.message : "" + t),
    e && "error" in e && (n = e.error));
  let r = Y(e),
    o = /^Unable to resolve specifier '(?<specifier>[^']*)'/u.exec(n);
  return (
    o?.groups?.specifier &&
      ((n = `Unable to resolve '${o.groups.specifier}'
...
function _(e, t, n) {
  return u(e) ? (e.type === "codeFile" ? `Error in ${B(t)}` : n ? `Error in ${n}` : y) : y;
}
function B(e) {
  return e.startsWith("./") ? e.replace("./", "") : e;
}
export { Z as a, P as b, H as c, K as d, re as e };
//# sourceMappingURL=chunk-4OPICYQK.mjs.map
